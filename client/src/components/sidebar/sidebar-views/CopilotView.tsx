...
function CopilotView() {
    ...
    const { generateCode, output, isRunning, setInput, input } = useCopilot()
    const { activeFile, updateFileContent, setActiveFile, openFiles } = useFileSystem()
    const { currentUser } = useAppContext()
    const { setMessages } = useChatRoom()

    const handleGenerateCode = async () => {
        // Add user question to chat messages
        if (input && input.length > 0) {
            ...
            setMessages((messages) => [...messages, userQuestion])
            
            // Generate code with file context and add AI response to chat
            await generateCode(
                questionText, 
    ...
    return (
        ...
        >
            ...
            <textarea
                ...
                placeholder="Ask me anything about your code, find errors, or generate new code..."
                value={input}
